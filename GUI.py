...
LOG_FORMATS = ["TXT", "CSV", "JSON"]
log_format = "TXT"  # Default, can be changed by user

def save_log(log_file, message, format="TXT"):
	try:
		if format == "TXT":
...
def unified_log(message, log_file=None, gui_callback=None, format=None):
	timestamp = datetime.now().strftime("[%Y-%m-%d %H:%M:%S]")
	full_message = f"{timestamp} {message}"
	fmt = format if format else log_format
	if log_file:
		save_log(log_file, full_message, fmt)
	if gui_callback:
...
def gui_log_callback(msg, tag=None):
	gui_log_queue.put((msg, tag))

def poll_gui_log(text_widget):
	while not gui_log_queue.empty():
		msg, tag = gui_log_queue.get()
		text_widget.config(state='normal')
		text_widget.insert('end', msg + '\n', tag if tag else "normal")
		text_widget.see('end')
		text_widget.config(state='disabled')
	text_widget.after(100, lambda: poll_gui_log(text_widget))
...
class TCPServerThread(threading.Thread):
	def __init__(self, interface, port, timeout, max_connections, trigger_bytes, log_file, log_callback, test_mode=False):
		...
		if not isinstance(interface, str) or not interface:
			raise ValueError("TCPServerThread: 'interface' must be a non-empty string")
		if not isinstance(port, int) or not (1 <= port <= 65535):
			raise ValueError("TCPServerThread: 'port' must be an integer between 1 and 65535")
		if not isinstance(timeout, int) or timeout < 1:
			raise ValueError("TCPServerThread: 'timeout' must be a positive integer")
		if not isinstance(max_connections, int) or max_connections < 1:
			raise ValueError("TCPServerThread: 'max_connections' must be a positive integer")
		if not (isinstance(trigger_bytes, bytes) or isinstance(trigger_bytes, str)):
			raise ValueError("TCPServerThread: 'trigger_bytes' must be bytes or hex string")
		if not isinstance(log_file, str) or not log_file:
			raise ValueError("TCPServerThread: 'log_file' must be a non-empty string")
	...
	def run(self):
		# Check for missing arguments and provide feedback
		missing_args = []
		if not self.interface:
			missing_args.append("interface")
		if not self.port:
			missing_args.append("port")
		if not self.timeout:
			missing_args.append("timeout")
		if not self.max_connections:
			missing_args.append("max_connections")
		if not self.trigger_bytes:
			missing_args.append("close_hex (trigger_bytes)")
		if not self.log_file:
			missing_args.append("log_file")
		if missing_args:
			msg = f"Cannot start TCP server: missing arguments: {', '.join(missing_args)}"
			# Use "error" tag for critical feedback
			if self.log_callback:
				self.log_callback(msg + "\n", "error")
			unified_log(msg, log_file=None)
			return
		...
		with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
			...
			# Always use unified_log for both file and GUI so timestamps are identical
			unified_log(
							...
							if ready[0]:
								try:
									...
									if self.test_mode and total_bytes > 10:
										msg = f"{addr[0]} Test mode: Closing connection after 10 bytes"
										unified_log(
											msg,
											log_file=self.log_file,
									...
									trigger_bytes = self.trigger_bytes if isinstance(self.trigger_bytes, bytes) else bytes.fromhex(self.trigger_bytes)
									if trigger_bytes in data:
										msg = f"{addr[0]} Disconnected (trigger matched)"
										unified_log(
											msg,
											log_file=self.log_file,
											gui_callback=lambda m: self.log_callback(m + "\n", "warning") if self.log_callback else None
									...
									msg = f"{addr[0]} Error receiving data: {e}"
									unified_log(
										msg,
										log_file=self.log_file,
								...
								msg = f"{addr[0]} Disconnected (timed out)"
								unified_log(
									msg,
									log_file=self.log_file,
									gui_callback=lambda m: self.log_callback(m + "\n", "warning") if self.log_callback else None
...
def create_server_frame(parent, title, default_file, service_type="tcp"):
	...
	# interface_combo will be defined later in the serial section of create_server_frame
	...
	def log_callback(msg, tag=None):
		# Determine tag for GUI coloring
		if tag is None:
			lower_msg = msg.lower()
			if "error" in lower_msg or "fail" in lower_msg:
				...
				tag = "normal"
		# Log to file (plain text) and GUI (with color)
	...
	def graceful_shutdown():
		...
		if server_thread and hasattr(server_thread, "stop"):
			server_thread.stop()
			server_thread = None
		if serial_thread and hasattr(serial_thread, "stop"):
			serial_thread.stop()
			serial_thread = None
		if serial_obj and hasattr(serial_obj, "close"):
			try:
				serial_obj.close()
			except:
				pass
		...
		if switch.instate(['selected']):
			if service_type == "tcp":
				...
				if not validate_port(port):
					...
					return
				if not validate_hex_string(close_hex):
					messagebox.showerror("Input Error", "Invalid Close Hex string.")
					switch.state(['!selected'])
					return
				try:
					...
					server_thread = TCPServerThread(interface_combo.get(), int(port), timeout, max_conn, trigger_bytes, file_entry.get(), gui_log_callback, test_mode.get())
					server_thread.start()
...
def launch_gui():
	...
	def load_last_settings():
		last_settings_path = os.path.join(script_dir, "last_settings.json")
	...
	def save_last_settings(config, layout_config=None):
		last_settings_path = os.path.join(script_dir, "last_settings.json")
		try:
	...
	def toggle_theme():
		...
		update_and_save_config({"theme": new_theme})
	...
	def save_config():
		...
		layout = {}
		try:
			layout["visible"] = {k: v.winfo_ismapped() for k, v in frame_refs.items()}
			layout["layout"] = "horizontal" if main_frame.grid_size()[0] > 1 else "vertical"
		except Exception:
			pass
	...
	def load_config():
		try:
			...
			layout_conf = config.get("layout", None)
			...
			tcp_widgets["close_hex"].insert(0, tcp.get("close_hex", "61 62 63 0D 0A"))
			if tcp_widgets["timeout"]: tcp_widgets["timeout"].delete(0, 'end'); tcp_widgets["timeout"].insert(0, tcp.get("timeout", "20"))
			if tcp_widgets["max_conn"]: tcp_widgets["max_conn"].delete(0, 'end'); tcp_widgets["max_conn"].insert(0, tcp.get("max_conn", "1"))
	...
	ttk.Button(top_frame, text="Theme", command=toggle_theme, bootstyle="secondary").pack(side='left', padx=5)
	...
	log_format_var = tk.StringVar(value=log_format)
	def set_log_format(fmt):
		global log_format
		log_format = fmt
		update_and_save_config({"log_format": fmt})
	format_frame = ttk.Frame(root, padding=5)
	format_frame.pack(fill='x')
	ttk.Label(format_frame, text="Log Format:").pack(side='left')
	for fmt in LOG_FORMATS:
		ttk.Radiobutton(format_frame, text=fmt, variable=log_format_var, value=fmt, command=lambda f=fmt: set_log_format(f)).pack(side='left')

		# Load last-used settings
		last_settings = load_last_settings()
		if last_settings:
			try:
				tcp = last_settings.get("tcp", {})
				udp = last_settings.get("udp", {})
				serial = last_settings.get("serial", {})
				layout_conf = last_settings.get("layout", None)
				tcp_widgets["interface"].set(tcp.get("interface", "0.0.0.0"))
				tcp_widgets["port"].delete(0, 'end')
				tcp_widgets["port"].insert(0, tcp.get("port", "1234"))
				tcp_widgets["close_hex"].delete(0, 'end')
	...
	# Poll log queue for GUI updates
	poll_gui_log(tcp_widgets["text_widget"])
	poll_gui_log(udp_widgets["text_widget"])
	poll_gui_log(serial_widgets["text_widget"])

	# On closing, save settings and shutdown threads/sockets
	def graceful_shutdown():
		# Attempt to stop any running server threads if they exist
		global tcp_widgets, udp_widgets, serial_widgets
